...
alter table slipproperties drop foreign key FKpxfygofj9rjg6peeflrhxgxjo;
...
create table opinionbase (dtype integer not null, opinionkey bigint not null, countreferringopinions integer not null, opiniondate date, title varchar(127), primary key (opinionkey)) ENGINE=InnoDB;
...
create table opinionstatutecitation (countreferences integer not null, statutecitation_sectionnumber char(32) not null, statutecitation_title char(3) not null, opinionbase_opinionkey bigint not null, primary key (opinionbase_opinionkey, statutecitation_sectionnumber, statutecitation_title)) ENGINE=InnoDB;
create table role (id bigint not null auto_increment, role varchar(255) not null, primary key (id)) ENGINE=InnoDB;
create table slipproperties (court varchar(15), disposition varchar(31), fileextension varchar(7), filename varchar(31), summary varchar(4008), slipopinion_opinionkey bigint not null, primary key (slipopinion_opinionkey)) ENGINE=InnoDB;
...
alter table slipproperties add constraint FKpxfygofj9rjg6peeflrhxgxjo foreign key (slipopinion_opinionkey) references opinionbase (opinionkey);
